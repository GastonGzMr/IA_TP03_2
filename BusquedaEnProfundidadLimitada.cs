...
namespace IA_TP03_2
{
    public class BusquedaEnProfundidadLimitada: Algoritmo
    {
        public int Profundidad { get; set; }

        public BusquedaEnProfundidadLimitada(int profundidad)
        {
        ...
        public override bool Resolver(IEnumerable<Camino> caminos, string ciudadInicial, string ciudadDestino)
        {
            ...
            while (!caminoActual.CiudadDeOrigen.Equals(ciudadDestino) && !caminoActual.CiudadDestino.Equals(ciudadDestino))
            {
                for (int i = 1; i <= Profundidad; i++)
                {
                    ...
                    if (caminoActual.CiudadDeOrigen.Equals(ciudadDestino) || caminoActual.CiudadDestino.Equals(ciudadDestino))
                    {
                        break;
                    }
                }
